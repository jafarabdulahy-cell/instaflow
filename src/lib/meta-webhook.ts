...
type InstagramAccountRecord = NonNullable<Awaited<ReturnType<typeof prisma.instagramAccount.findFirst>>>;
...
async function maybeSendLiveDmReply(input: {
  ...
  const response = await sendInstagramTextMessage({
    pageId: input.account.facebookPageId || undefined,
...
async function handleNewDM(account: InstagramAccountRecord, event: Record<string, unknown>) {
  ...
  const text = firstString(message.text, attachments.length ? "پیام رسانه‌ای جدید" : "", "پیام جدید");
...
async function handleChangeInteraction(account: InstagramAccountRecord, change: Record<string, unknown>) {
