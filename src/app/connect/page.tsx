...
import { ArrowRight, Camera, CheckCircle2, Link2, ShieldCheck, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top,#efe9ff_0,#ffffff_72%)] px-4 py-5 text-[#17112A]">
      <div className="mx-auto w-full max-w-[430px] animate-fade-in">
        <div className="mb-4 flex items-center justify-between gap-3">
          <Link
            href="/dashboard"
            className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-[#5B2BE2] shadow-sm ring-1 ring-[#ECE8F6]"
          >
        ...
        <section className="relative overflow-hidden rounded-[30px] bg-gradient-to-br from-[#5B2BE2] via-[#722FE8] to-[#8E58FF] p-5 text-white shadow-[0_22px_50px_rgba(91,43,226,0.24)]">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="flex items-center gap-1 text-[12px] font-black text-white/80">
                <Sparkles className="h-4 w-4" /> اتصال امن
              </p>
              <h1 className="mt-2 text-2xl font-black">اتصال پیج اینستاگرام</h1>
              <p className="mt-2 text-[12px] font-medium leading-6 text-white/86">
                اتصال از مسیر رسمی Meta، بدون دریافت رمز عبور شما.
              </p>
            </div>
        ...
        <div className="my-4 flex flex-wrap items-center justify-center gap-2">
          <Badge className="bg-green-50 text-green-700">
            <ShieldCheck className="ml-1 h-3.5 w-3.5" /> بدون پسورد
          </Badge>

          <Badge className="bg-[#F2EEFF] text-[#5B2BE2]">
            <ShieldCheck className="ml-1 h-3.5 w-3.5" /> OAuth رسمی
          </Badge>

          <Badge className="bg-pink-50 text-pink-700">
            <Camera className="ml-1 h-3.5 w-3.5" /> Creator / Business
          </Badge>
        </div>
            ...
            <div className="space-y-3">
              {steps.map((step, index) => (
                <div key={step} className="flex items-start gap-3">
                  <div className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#F2EEFF] text-xs font-black text-[#5B2BE2]">
                    {index + 1}
                  </div>
        ...
        <Card className="mt-4 rounded-[26px] border-0 bg-white shadow-[0_16px_38px_rgba(42,16,90,0.08)] ring-1 ring-[#ECE8F6]">
          <CardContent className="p-5">
            ...
            <div className="grid gap-2">
              {["دریافت و پاسخ DM", "خواندن اطلاعات پایه پیج", "ثبت Webhook", "آمار و گزارش‌های پیج"].map(
                (permission) => (
                  <div
                    key={permission}
                    className="flex items-center gap-2 rounded-2xl bg-[#FAF9FF] px-3 py-2 text-[13px] font-bold text-[#5F596F]"
                  >
        ...
        <p className="mt-4 text-center text-[12px] font-medium leading-6 text-[#8A8498]">
