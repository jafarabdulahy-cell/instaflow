...
import { ArrowRight, Home, Link2, Loader2, MessageCircle, MessageSquare, Plus, Send, Trash2, UsersRound, Zap } from "lucide-react";

type Rule = { id: string; name: string; triggers: string[]; matchType: "equals" | "contains"; publicReply: string; dmReply: string; isActive: boolean; sendDm: boolean };
...
export default function CommentsPage() {
  ...
  async function save() {
    setMessage("");
    const res = await fetch("/api/automation/comment-rules", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, triggers, matchType, publicReply, dmReply, sendDm, isActive: true }) });
    const json = await res.json().catch(() => ({}));
        ...
        <section className="space-y-3">
          {rules.map((rule) => (
            <article key={rule.id} className="rounded-[24px] bg-white p-3 text-right shadow-[0_12px_28px_rgba(42,16,90,0.06)] ring-1 ring-[#ECE8F6]">
              ...
              <p className="mt-3 rounded-2xl bg-[#FBFAFF] p-3 text-[11px] font-bold leading-6 text-[#6D6780] ring-1 ring-[#ECE8F6]">کامنت: {rule.publicReply}<br />دایرکت: {rule.dmReply || "—"}</p>
            </article>
          ))}
        </section>
      </main>
